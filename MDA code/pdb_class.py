...
def prepare_secondary_structure_file(pdb_file, phi_data):
	'''
	@description
		...
		5. Rename structure
	@input
		pdb_file - PDB file from RCSB, ex. 4eiy.pdb
		phi_file - secStructure file, ex. 4eiy.phi
...
def prepare_rsa_file(pdb_file, phi_data):
	'''
	@description
	...
	@input
		pdb_file - PDB file from RCSB, ex. 4eiy.pdb
		phi_file - secStructure file, ex. 4eiy.phi
		phi_file - processed PHI file, list of ASG lines
	@output 
	...
	# phifile = open(phi_file, 'r').readlines()
	# phi_data = phi_file
	with open(pdb_file.replace('.pdb','.rsa'), 'w') as rsafile:
...
def find_ligand_atom_bonds(file1, residues, u1_names, ligand_centroids, chain, coords):
	'''
	@description
...
def find_centroid_ligand_bonds(file1, centroid_coords, ligand_centroids):
	'''
	@description
...
if __name__ == '__main__':
	...
	# PDB.split('/')[1].replace('.pdb', '_bonds')
	...
	# print('Prepare secondaryStructure', time.time()-t0)

	### MAIN ###
	# Define mda.objects and create array of AminoAcid classes
	u = mda.Universe(PDB)
	prot = u.select_atoms('protein')	# u1 = u.select_atoms('protein and not name OXT')
	...
	prot = prot.atoms #define for each atom of protein
	prot_resnames, prot_resids, prot_segids, prot_atoms = prot.resnames, prot.resids, prot.segids, prot.names

	residues = [(res+':'+str(rid)+':'+sid) for res, rid, sid in zip(prot_resnames, prot_resids, prot_segids)] #like HIS:26:A
	coords = {res+':'+atom : pos for res, atom, pos in zip(residues, prot_atoms, prot.positions)} #like {HIS:26:A:N : array([2.61,1.454,10.018]}
	...
	# residues = [(i+':'+str(j)+':'+k) for i, j, k in zip(u1_atoms.resnames, u1_atoms.resids, u1_atoms.segids)]
	...
	h.run()
	h.generate_table() 
	find_hydrogen_bonds(h, PDB, prot_segids, coords, chain)
	# print('Generate hydrogen table + define chain list', time.time()-t0)

	vdw_coords = prot.positions
	pairs, distances = mda.lib.distances.capped_distance(vdw_coords, vdw_coords, max_cutoff=4.25, min_cutoff=0)
	find_vdw_bonds(pairs, PDB, prot_resnames, prot_resids, prot_segids, prot_atoms, coords, chain)
	...
	find_ligand_atom_bonds(PDB, residues, prot_atoms, ligand_centroids, chain, coords)
	find_centroid_bonds(PDB, centroid_coords)
	...
	# # ligands = allatoms - protein - hoh_atoms - metalls
	# # ligandCentroids = ligands.center_of_geometry(compound='residues')	# # ligandCentroids  = dict(zip(prs.getTotalResidue(ligands, flag=False), ligandCentroids))
