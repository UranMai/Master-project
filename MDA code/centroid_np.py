...
def make_undirected_interactions(data):
    '''
    consider undirected interactions when we use np.argsort
    create pairs 
    select subdata of all interactions with certain pair of acids and calculate the sum of weight
    '''
...
if __name__ == '__main__':
	...
	try:        
	    ...
	    # bfactor = np.loadtxt(re.sub('.pdb','_Bfactor',pdb), dtype=np.str)

	    liganddat = np.loadtxt(re.sub('.pdb','_ligands', tf), dtype=np.str)
	    if len(liganddat) != 0:
	        ligandmode = True
	    ligandCentroidFile = np.loadtxt(re.sub('.pdb','', tf)+'_centroidNetLigand', dtype=np.str)
	    
	    terminalAtoms = {}
	    with open('../Test_data/uniqueAtoms', 'r') as terminalAtomFile:
	...
	# Select acids(nodes) from written interactions in net file 
	...
	# Consider only undirected interactions excluding PP bonds 
	basedata_noPP = make_undirected_interactions(basedata[basedata[:,4]!='PP'])
	...
	secondOrderDegree, secondOrderDegree_sidechain = first_second_order_degree(influencenet, influencenet_directed, nodes)
	...
	# print(outZ)
	# final_out.to_csv(re.sub('.pdb', '', tf)+'_scoresCentroid', sep='\t', index=False)
	# outZ.to_csv(re.sub('.pdb', '', tf)+'_scoresCentroidZ', sep='\t', index=False)
	final_out.to_csv(re.sub('.pdb', '', tf)+'_scoresCentroid', sep='\t', index=False)
	outZ.to_csv(re.sub('.pdb', '', tf)+'_scoresCentroidZ', sep='\t', index=False)